...
Outputs both gradient attribution scores and predicted half-life values.
"""
...
def main():
  ...
  for fi in range(num_folds):
    for ci in range(num_crosses):
      ...
      # predict gradients and half-life for each sequence
      scores_fc = []
      preds_fc = []
      for si in range(num_seqs):
        scores_fc.append(seqnn_model.gradients(seqs_1hot[si]))
        preds_fc.append(seqnn_model.predict(seqs_1hot[si:si+1]))  # predict expects a batch dim
      scores.append(scores_fc)
      preds.append(preds_fc)

  # scores shape: (num_folds*num_crosses, num_seqs, seq_len, 6)
