...
def color_by_plddt(obj_name):
    ...
    the broader one below it. Uses the plddt_* colors defined above
    (rather than builtin PyMOL color names) so the exact shades are
    guaranteed regardless of PyMOL version/build.
    """
...
def write_plddt_pml(cif_path, pml_path, obj_name):
    """
    Standard mmCIF has no field for "PyMOL display color" -- only the
    B-factor column (already holding the pLDDT values) survives a plain
    File > Open of the .cif. So write a small companion .pml script next
    to the .cif that (re)defines the exact confidence colors and applies
    them on load. Opening *this* file in PyMOL (File > Open, or
    `pymol thisfile.pml` from the command line) reproduces the coloring
    exactly, since it re-runs the same commands used to generate the
    PNG renders.
    """
    cif_basename = os.path.basename(cif_path)
    script = f"""\
# Auto-generated by compare_af3_candidates.py -- reproduces the discrete
# pLDDT confidence coloring for {cif_basename}.
# Usage: PyMOL GUI -> File > Open -> select this .pml (not the .cif)
#        or from a terminal: pymol {os.path.basename(pml_path)}

load {cif_basename}, {obj_name}
hide everything, {obj_name}
show cartoon, {obj_name}
set cartoon_ring_mode, 3, {obj_name}
set cartoon_ring_finder, 1, {obj_name}

set_color slate, [0.0, 0.32, 0.84]
set_color aquamarine, [0.25, 0.88, 0.82]
set_color yelloworange, [1.0, 0.86, 0.07]
set_color deepsalmon, [0.86, 0.08, 0.24]

color deepsalmon, {obj_name}
color yelloworange, {obj_name} and b > 50
color aquamarine, {obj_name} and b > 70
color slate, {obj_name} and b > 90
...
# column) plus a companion .pml script that reproduces the exact
# coloring when the .cif is (re)loaded -- see write_plddt_pml() above
# for why the .cif alone can't carry PyMOL's display colors.
if has_plddt_a:
    color_by_plddt("structA")
    cmd.save(plddt_cif1, "structA")
    write_plddt_pml(plddt_cif1, os.path.splitext(plddt_cif1)[0] + ".pml", "structA")
    cmd.disable("structB")
    cmd.orient("structA")
...
if has_plddt_b:
    ...
    write_plddt_pml(plddt_cif2, os.path.splitext(plddt_cif2)[0] + ".pml", "structB")
    cmd.disable("structA")
    cmd.orient("structB")
...
# --------------------------------------------------------------------------
# Companion .pml script: reproduces the pLDDT coloring on a bare .cif
# --------------------------------------------------------------------------
#
# Plain mmCIF (unlike a PyMOL .pse session) has no field for a viewer's
# display color -- only numeric per-atom data like B-factor (where we
# stored pLDDT) survives. So dragging the saved *_plddt.cif straight
# into a fresh PyMOL window will show it in PyMOL's default coloring,
# not the confidence scheme. This companion script is what recreates it.

_PLDDT_PML_TEMPLATE = """\
# Auto-generated by compare_af3_candidates.py
#
# Reproduces the discrete AlphaFold-style pLDDT confidence coloring
# (pLDDT is stored in the B-factor column of the paired .cif). Keep
# this file next to {cif_name}.
#
# Usage:
#   From a terminal:            pymol {pml_name}
#   Inside a running PyMOL GUI: File > Run Script...  ->  pick this file
#                                (or type `run {pml_name}` on the PyMOL
#                                command line)

load {cif_name}, {obj_name}
hide everything
show cartoon
...
orient {obj_name}
zoom {obj_name}, buffer=5
"""


def write_plddt_pml(cif_path: str, pml_path: str, obj_name: str) -> None:
    """
    Write a small companion PyMOL script (.pml) alongside cif_path that
    loads it and re-applies the discrete pLDDT confidence coloring.
    Run it (rather than opening the .cif directly) to see the coloring
    in a fresh PyMOL session -- see module docstring above for why.
    """
    text = _PLDDT_PML_TEMPLATE.format(
        cif_name=os.path.basename(cif_path),
        pml_name=os.path.basename(pml_path),
        obj_name=obj_name,
    )
    with open(pml_path, "w") as fh:
        fh.write(text)
...
def pymol_super_and_render(cif1: str, cif2: str, outdir: str,
    ...
    # Plain mmCIF has no field for a viewer's display color -- only the
    # B-factor column (pLDDT) travels with the file. So opening the
    # saved .cif fresh in PyMOL will NOT reproduce the coloring by
    # itself; write a companion .pml script next to it that does.
    if result.get("plddt_colored_A"):
        pml1 = os.path.splitext(plddt_cif1)[0] + ".pml"
        write_plddt_pml(plddt_cif1, pml1,
                         obj_name=os.path.splitext(os.path.basename(plddt_cif1))[0])
        print(f"  Companion coloring script    : {pml1}")
    if result.get("plddt_colored_B"):
        pml2 = os.path.splitext(plddt_cif2)[0] + ".pml"
        write_plddt_pml(plddt_cif2, pml2,
                         obj_name=os.path.splitext(os.path.basename(plddt_cif2))[0])
        print(f"  Companion coloring script    : {pml2}")
...
def write_report(path: str, name1, name2, dssr1: DssrResult, dssr2: DssrResult,
    ...
    with open(path, "w") as fh:
        ...
        if plddt_cif1 and os.path.exists(plddt_cif1):
            ...
            pml1 = os.path.splitext(plddt_cif1)[0] + ".pml"
            if os.path.exists(pml1):
                fh.write(f"  {name1} coloring script (run this, not the "
                          f"bare .cif, to see the colors): {pml1}\n")
        ...
        if plddt_cif2 and os.path.exists(plddt_cif2):
            ...
            pml2 = os.path.splitext(plddt_cif2)[0] + ".pml"
            if os.path.exists(pml2):
                fh.write(f"  {name2} coloring script (run this, not the "
                          f"bare .cif, to see the colors): {pml2}\n")
...
def main():
    ...
    plddt_cif1 = os.path.join(args.outdir, f"{name1}_plddt.cif")
    plddt_cif2 = os.path.join(args.outdir, f"{name2}_plddt.cif")
