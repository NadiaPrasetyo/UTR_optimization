...
#SBATCH --gres=gpu:1
#SBATCH --array=0-9
#SBATCH --output=optimization_a7/patent_dist_optimization/gen_0001/af3_jobs/batch_0000/logs/af3_%A_%a.log

module purge
module load alphafold3/3.0.3

export AF3_DB=/opt/alphafold3/databases
export AF3_WD=optimization_a7/patent_dist_optimization/gen_0001
...
LINES=$(awk -F'\t' -v s="$SLOT_START" -v e="$SLOT_END" '$1 >= s && $1 <= e' "optimization_a7/patent_dist_optimization/gen_0001/af3_jobs/batch_0000/jobs.tsv")
...
gpu_healthy() {
    if ! nvidia-smi --query-gpu=index,name,pstate,memory.used,memory.total \
            --format=csv,noheader > "optimization_a7/patent_dist_optimization/gen_0001/af3_jobs/batch_0000/logs/nvidia_smi_${SLURM_ARRAY_JOB_ID}_${SLURM_ARRAY_TASK_ID}.log" 2>&1; then
...
" 2>&1)
    rc=$?
    echo "$probe_out" >> "optimization_a7/patent_dist_optimization/gen_0001/af3_jobs/batch_0000/logs/nvidia_smi_${SLURM_ARRAY_JOB_ID}_${SLURM_ARRAY_TASK_ID}.log"
    if [ $rc -eq 1 ]; then
...
run_one() {
    ...
    local attempt=1
    local task_log="optimization_a7/patent_dist_optimization/gen_0001/af3_jobs/batch_0000/logs/af3_task_${job_name}.log"
