...
#SBATCH --gres=gpu:1
#SBATCH --array=0-9
#SBATCH --output=optimization_a7/af3_optimization/gen_0031/af3_jobs/batch_0000/logs/af3_%A_%a.log

module purge
module load alphafold3/3.0.3

export AF3_DB=/opt/alphafold3/databases
export AF3_WD=optimization_a7/af3_optimization/gen_0031
...
LINES=$(awk -F'\t' -v s="$SLOT_START" -v e="$SLOT_END" '$1 >= s && $1 <= e' "optimization_a7/af3_optimization/gen_0031/af3_jobs/batch_0000/jobs.tsv")
...
run_one() {
    ...
    af3 "$json_path" "$out_dir" --run_data_pipeline=true --run_inference=true \
        > "optimization_a7/af3_optimization/gen_0031/af3_jobs/batch_0000/logs/af3_task_${job_name}.log" 2>&1
    local rc=$?
    if [ $rc -ne 0 ]; then
        echo "[$job_name] FAILED at $(date) (exit $rc) — see optimization_a7/af3_optimization/gen_0031/af3_jobs/batch_0000/logs/af3_task_${job_name}.log"
